...
window.addEventListener('load', () => {
    ...
    if (storedData) {
        ...
        console.log(listDishOrdered[tableNumber]);
        
        // check price and code of dish
        for (let indexOfListDishOrdered = 0; indexOfListDishOrdered < listDishOrdered[tableNumber].length; indexOfListDishOrdered++){
            ...
            for (let indexOfdishAndPrice = 0; indexOfdishAndPrice < dishAndPrice.length; indexOfdishAndPrice++){
                if (listDishOrdered[tableNumber][indexOfListDishOrdered] === dishAndPrice[indexOfdishAndPrice][1]){
                    priceDish[indexOfListDishOrdered] = dishAndPrice[indexOfdishAndPrice][2];
                    codeOfDish[indexOfListDishOrdered] = dishAndPrice[indexOfdishAndPrice][0];
                    break;
                }
            }
        }

        // calc total and discount of bill
        for (let indexOfListDishOrdered = 0; indexOfListDishOrdered < listDishOrdered[tableNumber].length; indexOfListDishOrdered++){
            totalCheck += priceDish[indexOfListDishOrdered] * listQuantity[tableNumber][indexOfListDishOrdered]
        }
        if (discountCode <= 100){
        ...
        for (let indexOfListDishOrdered = 0; indexOfListDishOrdered < listDishOrdered[tableNumber].length; indexOfListDishOrdered++){
            products.push({name: listDishOrdered[tableNumber][indexOfListDishOrdered], quantity: listQuantity[tableNumber][indexOfListDishOrdered], price: priceDish[indexOfListDishOrdered]});

            date = dateOfTable[tableNumber][0];
            time = timeOfTable[tableNumber][0];
            code = codeOfDish[indexOfListDishOrdered];
            nameDish = listDishOrdered[tableNumber][indexOfListDishOrdered];
...
btnPrint.addEventListener('click', () => {
    // Sử dụng window.print() để in ấn nội dung trang
    window.print();
});
